...
const surveyOfferSchema = new mongoose.Schema({
  ...
  category: {
    type: String,
    enum: ['finance', 'shopping', 'entertainment', 'technology', 'health', 'travel', 'education', 'other'],
    default: 'other'
  },
  coinReward: {
    type: Number,
...
surveyOfferSchema.pre('save', function(next) {
  ...
  if (this.analytics.views > 0) {
    this.analytics.conversionRate = (this.analytics.completions / this.analytics.views) * 100;
  }
...
surveyOfferSchema.methods.isEligibleForUser = function(userProfile) {
  ...
  if (this.targetAudience.countries && this.targetAudience.countries.length > 0) {
...
surveyOfferSchema.methods.recordCompletion = function(completionTime, coinsIssued) {
