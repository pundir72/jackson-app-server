...
ticketSchema.methods.getTimeToResponse = function () {
  ...
  const responseDeadline = new Date(
    this.createdAt.getTime() + this.sla.response_time * 60 * 60 * 1000
  );
...
ticketSchema.methods.getTimeToResolution = function () {
  ...
  const resolutionDeadline = new Date(
    this.createdAt.getTime() + this.sla.resolution_time * 60 * 60 * 1000
  );
  return resolutionDeadline;
};

ticketSchema.methods.addInternalNote = function (
  note,
  userId,
  isPrivate = true
) {
...
ticketSchema.methods.getDisplayData = function () {
  return {
    ...
    user: this.user,
    game:
      this.game && typeof this.game === "object" && this.game.title
        ? {
...
ticketSchema.pre("save", function (next) {
  ...
  if (this.description) {
    this.wordCount = this.description
      .split(/\s+/)
