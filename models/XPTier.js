...
// Returns the highest tier (highest xpMin) that matches the XP value
// If XP exceeds all tier max values, returns the highest tier
xpTierSchema.statics.findByXpValue = async function (xp) {
  // First, try to find a tier where XP falls within the range
  let tier = await this.findOne({
    xpMin: { $lte: xp },
    xpMax: { $gte: xp },
    status: true,
  }).sort({ xpMin: -1 }); // Sort descending to get the highest matching tier first

  // If no tier found (XP exceeds all max values), return the highest tier
  if (!tier) {
    tier = await this.findOne({
      status: true,
    }).sort({ xpMax: -1, xpMin: -1 }); // Get tier with highest xpMax, then highest xpMin
  }
