...
router.get('/challenges', adminAuth, [
  ...
  query('type').optional().isIn(['spin', 'game', 'survey', 'referral', 'watch_ad', 'social_share', 'app_install', 'quiz', 'custom']).withMessage('Invalid challenge type'),
  query('status').optional().isIn(['scheduled', 'live', 'completed', 'expired', 'draft']).withMessage('Invalid status'),
...
router.get('/challenges/calendar', adminAuth, [
  query('year').optional().isInt({ min: 2020, max: 2030 }).withMessage('Year must be between 2020 and 2030'),
  query('month').optional().isInt({ min: 0, max: 11 }).withMessage('Month must be between 0 and 11'),
  query('type').optional().isIn(['spin', 'game', 'survey', 'referral', 'watch_ad', 'social_share', 'app_install', 'quiz', 'custom']).withMessage('Invalid challenge type'),
  query('status').optional().isIn(['scheduled', 'live', 'completed', 'expired', 'draft']).withMessage('Invalid status')
], async (req, res) => {
  try {
    ...
    const challenges = await DailyChallenge.getCalendarView(targetYear, targetMonth, filters);
...
router.post('/challenges', adminAuth, [
  body('challengeDate').isISO8601().withMessage('Challenge date must be valid ISO 8601 date'),
  body('title').notEmpty().withMessage('Title is required'),
  body('type').isIn(['spin', 'game', 'survey', 'referral', 'watch_ad', 'social_share', 'app_install', 'quiz', 'custom']).withMessage('Invalid challenge type'),
...
router.put('/challenges/:id', adminAuth, [
  ...
  body('type').optional().isIn(['spin', 'game', 'survey', 'referral', 'watch_ad', 'social_share', 'app_install', 'quiz', 'custom']).withMessage('Invalid challenge type'),
...
router.patch('/challenges/:id/visibility', adminAuth, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Challenge ${challenge.isVisible ? 'shown' : 'hidden'} successfully`,
...
router.get('/bonus-days/:dayNumber', adminAuth, async (req, res) => {
  try {
    const bonusDay = await BonusDay.findByDayNumber(parseInt(req.params.dayNumber));
...
router.put('/bonus-days/:dayNumber', adminAuth, [
  body('title').notEmpty().withMessage('Title is required'),
  body('primaryReward.type').isIn(['coins', 'xp', 'giftcard', 'premium_features', 'bonus_spins', 'vip_access']).withMessage('Invalid primary reward type'),
  body('primaryReward.value').isInt({ min: 0 }).withMessage('Primary reward value must be a non-negative integer')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
router.post('/bonus-days/:dayNumber/preview', adminAuth, [
  body('previewType').isIn(['notification', 'banner']).withMessage('Preview type must be notification or banner')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
// Create pause rule
router.post('/pause-rules', adminAuth, [
  body('ruleName').notEmpty().withMessage('Rule name is required'),
  body('actionOnMiss').isIn(['pause_streak', 'reset_streak', 'grace_period', 'fallback_reward', 'no_action']).withMessage('Invalid action on miss'),
  body('graceDays').optional().isInt({ min: 0, max: 7 }).withMessage('Grace days must be between 0 and 7'),
  body('impactOnXP').optional().isBoolean().withMessage('Impact on XP must be boolean'),
  body('resetCoins').optional().isBoolean().withMessage('Reset coins must be boolean')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
// Update pause rule
router.put('/pause-rules/:id', adminAuth, [
  body('ruleName').optional().notEmpty().withMessage('Rule name cannot be empty'),
  body('actionOnMiss').optional().isIn(['pause_streak', 'reset_streak', 'grace_period', 'fallback_reward', 'no_action']).withMessage('Invalid action on miss'),
  body('graceDays').optional().isInt({ min: 0, max: 7 }).withMessage('Grace days must be between 0 and 7'),
  body('impactOnXP').optional().isBoolean().withMessage('Impact on XP must be boolean'),
  body('resetCoins').optional().isBoolean().withMessage('Reset coins must be boolean')
], async (req, res) => {
...
router.patch('/pause-rules/:id/status', adminAuth, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Pause rule ${pauseRule.isActive ? 'activated' : 'deactivated'} successfully`,
