 ...
 * @access  Admin
 */
router.post('/rewards', adminAuth, upload.single('icon'), [
    body('name').notEmpty().withMessage('Reward name is required'),
    body('type').isIn(['coins', 'xp', 'coupon', 'bonus_task', 'premium_feature']).withMessage('Invalid reward type'),
    ...
    body('eligibleTiers').optional().isArray().withMessage('Eligible tiers must be an array')
], async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
        ...
        if (Number.isNaN(amountNum) || Number.isNaN(probabilityNum)) {
            return res.status(400).json({ success: false, error: 'Amount and probability must be numbers' });
        }
        ...
        let tiers = req.body.eligibleTiers ?? req.body['eligibleTiers[]'] ?? req.body.tierVisibility;
        if (!tiers) {
            tiers = TIER_LIST;
        ...
        // Normalize type
        const typeMap = { Coins: 'coins', coins: 'coins', XP: 'xp', xp: 'xp', Coupon: 'coupon', coupon: 'coupon', 'Bonus Task': 'bonus_task', bonus_task: 'bonus_task', Premium: 'premium_feature', premium: 'premium_feature', premium_feature: 'premium_feature' };
        ...
        const payload = {
            ...
            isActive: req.body.isActive !== undefined ? req.body.isActive : (req.body.active !== undefined ? req.body.active : true)
        };
        ...
        if (req.file) {
            ...
            if (isRaster) {
                const optimizedName = `${Date.now()}-optimized-${path.parse(req.file.filename).name}.webp`;
                const optimizedPath = path.join(iconsUploadDir, optimizedName);
                await sharp(req.file.path).resize(256, 256, { fit: 'inside' }).webp({ quality: 85 }).toFile(optimizedPath);
        ...
        const updatedProbabilityCheck = await SpinWheelReward.validateTotalProbability();
 ...
 * @access  Admin
 */
router.put('/rewards/:id', adminAuth, upload.single('icon'), [
    body('name').optional().notEmpty().withMessage('Reward name cannot be empty'),
    body('type').optional().isIn(['coins', 'xp', 'coupon', 'bonus_task', 'premium_feature']).withMessage('Invalid reward type'),
    body('amount').optional().isInt({ min: 0 }).withMessage('Amount must be non-negative'),
    ...
    try {
        ...
        if (req.body.probability !== undefined) {
            ...
            if (Number.isNaN(newProbability)) {
                return res.status(400).json({ success: false, error: 'Probability must be a number' });
            }
            const probabilityCheck = await SpinWheelReward.validateTotalProbability();
            const adjustedTotal = Number(probabilityCheck.totalProbability) - currentProbability + newProbability;
        ...
        if (updatePayload.label && !updatePayload.name) updatePayload.name = updatePayload.label;
        if (updatePayload.active !== undefined && updatePayload.isActive === undefined) updatePayload.isActive = updatePayload.active;
        if (updatePayload.amount !== undefined) updatePayload.amount = Number(updatePayload.amount);
        if (updatePayload.probability !== undefined) updatePayload.probability = Number(updatePayload.probability);

        // Normalize eligible tiers arrays
        if (updatePayload['eligibleTiers[]'] && !updatePayload.eligibleTiers) {
            updatePayload.eligibleTiers = Array.isArray(updatePayload['eligibleTiers[]'])
        ...
        if (updatePayload.tierVisibility && !updatePayload.eligibleTiers) {
            const TIER_LIST = ['Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond'];
            updatePayload.eligibleTiers = Array.isArray(updatePayload.tierVisibility)
                ? updatePayload.tierVisibility
                : [updatePayload.tierVisibility];
            if (updatePayload.eligibleTiers.includes('All Tiers')) updatePayload.eligibleTiers = TIER_LIST;
        ...
        if (req.file) {
            ...
            if (reward.icon && reward.icon.startsWith(config.IMAGE_BASE_URL)) {
                const oldPath = path.join(__dirname, '..', reward.icon.replace(config.IMAGE_BASE_URL, '').replace(/^\//, ''));
 ...
 * @access  Admin
 */
router.post('/rewards/:id/icon', adminAuth, upload.single('icon'), async (req, res) => {
    try {
        const reward = await SpinWheelReward.findById(req.params.id);
        if (!reward) {
        ...
        res.json({ success: true, message: 'Icon uploaded successfully', data: { iconUrl: publicUrl, rewardId: reward._id } });
    } catch (error) {
...
router.get('/config', adminAuth, async (req, res) => {
    try {
        ...
        if (!config) {
            ...
            return res.json({
                ...
                data: {
                    ...
                    cooldownPeriod: Math.floor(defaultConfig.cooldownMinutes / 60) * 60 === defaultConfig.cooldownMinutes ? defaultConfig.cooldownMinutes / 60 : defaultConfig.cooldownMinutes, // hours if already hours
        ...
        const TIER_LIST = ['Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond'];
        const isAllTiers = Array.isArray(config.eligibleTiers) && config.eligibleTiers.length >= 3 && config.eligibleTiers.every(t => TIER_LIST.includes(t));
        res.json({
            success: true,
            data: {
...
router.post('/config', adminAuth, [
    body('name').optional().notEmpty().withMessage('Configuration name is required')
], async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
        ...
        const spinMode = spinModeInput === 'ad-based' ? 'ad_based' : spinModeInput; // map frontend to backend enum
        ...
        const cooldownMinutes = Number.isFinite(cooldownPeriod) ? cooldownPeriod : Number(req.body.cooldownMinutes || 360);

        const maxSpinsPerDay = Number(req.body.maxSpinsPerDay || 3);

        const TIER_LIST = ['Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond'];
        let eligibleTiers = req.body.eligibleTiers || req.body['eligibleTiers[]'];
        if (!eligibleTiers) eligibleTiers = req.body.eligibleTiers || req.body.tiers || [];
        if (typeof eligibleTiers === 'string') eligibleTiers = [eligibleTiers];
        if (eligibleTiers.includes('All Tiers') || eligibleTiers.length === 0) eligibleTiers = TIER_LIST;
...
router.get('/analytics/overview', adminAuth, async (req, res) => {
    try {
        ...
        const winRate = totalSpins > 0 ? ((totalWins / totalSpins) * 100).toFixed(2) : 0;
        ...
        res.json({
            ...
            data: {
                overview: {
                    ...
                    averageSpinsPerDay: dailySpins.length > 0 
                        ? (dailySpins.reduce((sum, day) => sum + day.spinCount, 0) / dailySpins.length).toFixed(2)
                        : 0
                },
                rewardStats,
                dailySpins,
 ...
 * @access  Admin
 */
router.get('/analytics/spins', adminAuth, [
    query('page').optional().isInt({ min: 1 }),
    query('limit').optional().isInt({ min: 1, max: 100 })
], async (req, res) => {
    try {
