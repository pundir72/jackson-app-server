...
router.get('/rates', protect, async (req, res) => {
  try {
    ...
    const rate = CONVERSION_CONFIG.conversionRates[currency] || CONVERSION_CONFIG.conversionRates.USD;
...
router.post('/start-task', protect, async (req, res) => {
  try {
    ...
    if (!coins || coins < CONVERSION_CONFIG.minCoins || coins > CONVERSION_CONFIG.maxCoins) {
    ...
    const conversionId = `CONV-${Date.now()}-${Math.floor(Math.random() * 100000)}`;
...
router.post('/complete-task', protect, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.userId).select('wallet conversionSessions');
  ...
  try {
    ...
    res.json({
      ...
      data: {
        conversionId,
        status: 'ad_completed',
        message: 'Ad completed! You can now claim your conversion reward with bonus!'
      }
    });
  } catch (error) {
...
router.get('/history', protect, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.userId).select('conversionSessions');
