...
router.get('/week', protect, async (req, res) => {
  try {
    ...
    if (!userCreatedAt) {
      console.log(`⚠️ User ${req.user.userId} has no createdAt, using current date`);
      userCreatedAt = new Date();
    } else if (typeof userCreatedAt === 'string') {
      console.log(`⚠️ User ${req.user.userId} createdAt is string, converting to Date`);
      userCreatedAt = new Date(userCreatedAt);
      if (isNaN(userCreatedAt.getTime())) {
        console.log(`❌ Invalid date string for user ${req.user.userId}, using current date`);
        userCreatedAt = new Date();
      }
    } else if (!(userCreatedAt instanceof Date)) {
      console.log(`⚠️ User ${req.user.userId} createdAt is not Date object, converting`);
      userCreatedAt = new Date(userCreatedAt);
      if (isNaN(userCreatedAt.getTime())) {
        console.log(`❌ Cannot convert createdAt for user ${req.user.userId}, using current date`);
        userCreatedAt = new Date();
      }
    }
    ...
    if (cfg.weeklyMultiplier?.enabled && weekNumber > 1) {
      const multiplierType = cfg.weeklyMultiplier.type || 'gradual';
      if (multiplierType === 'gradual') {
        const increment = cfg.weeklyMultiplier.increment || 0.1;
        weekMultiplier = 1.0 + ((weekNumber - 1) * increment);
      }
    }
    ...
    const isBigRewardEligible = progress.days.slice(0, 6).every(d => d.status === 'claimed');
...
router.post('/claim', protect, async (req, res) => {
  try {
    ...
    if (!userCreatedAt) {
      ...
      userCreatedAt = new Date(userCreatedAt);
      if (isNaN(userCreatedAt.getTime())) {
        console.log(`❌ Invalid date string for user ${userId}, using current date`);
        userCreatedAt = new Date();
      }
    } else if (!(userCreatedAt instanceof Date)) {
      ...
      userCreatedAt = new Date(userCreatedAt);
      if (isNaN(userCreatedAt.getTime())) {
        console.log(`❌ Cannot convert createdAt for user ${userId}, using current date`);
        userCreatedAt = new Date();
      }
    }
    ...
    if (day.dayNumber === 7 && cfg.bigReward?.enabled !== false) {
      ...
      if (downgradeOnMiss) {
        const allDays1to6Claimed = progress.days.slice(0, 6).every(d => d.status === 'claimed');
    ...
    const { finalXP: finalXPWithTier, multiplier: tierMultiplier } = await applyTierMultiplierToXPV2(user, xp || 0);
    ...
    if (day.dayNumber < 7) {
      const nextDay = progress.days.find(d => d.dayNumber === day.dayNumber + 1);
