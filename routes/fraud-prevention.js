...
const verisoul = new VerisoulSDK(process.env.VERISOUL_API_KEY, process.env.VERISOUL_BASE_URL);
...
router.post('/test/session/authenticate', [
...
router.post('/session/unauthenticate', [
  auth,
...
router.delete('/list/:listName/account/:accountId', [auth], async (req, res) => {
  try {
    const { listName, accountId } = req.params;
...
router.post('/liveness/verify-face', [
  auth,
  body('sessionId').notEmpty().withMessage('Session ID is required')
], async (req, res) => {
  try {
...
// Verify identity
router.post('/liveness/verify-identity', [
  auth,
  body('sessionId').notEmpty().withMessage('Session ID is required'),
  body('accountId').notEmpty().withMessage('Account ID is required')
], async (req, res) => {
...
// Verify ID document
router.post('/liveness/verify-id', [
  auth,
  body('sessionId').notEmpty().withMessage('Session ID is required')
], async (req, res) => {
  try {
