...
async function getStreakConfig() {
  ...
  if (
    streakConfigCache &&
    streakConfigCacheTime &&
    now - streakConfigCacheTime < CACHE_DURATION
  ) {
  ...
  try {
    ...
    console.error(
      "Error loading streak config from database, using defaults:",
      error,
    );
...
router.get("/bonus-days", protect, async (req, res) => {
  try {
    ...
    const bonusDaysWithStatus = allBonusDays
      .map((bonusDay) => {
        ...
        const daysRemaining = Math.max(
          0,
          bonusDay.conditions.minStreak - currentStreak,
        );
        ...
        const isEligible = bonusDay.isEligibleForUser(
          userProfileWithCompletedTasks,
        );
        ...
        if (bonusDay.conditions.requiresCompletion !== false) {
          ...
          if (!completedTasksSet.has(todayStr)) {
            ...
            console.log(
              `[BONUS-PROGRESS] Bonus Day ${bonusDay.dayNumber}: Progress = 0% (today not completed)`,
            );
          } else {
            ...
            for (let i = 0; i < bonusDay.conditions.minStreak; i++) {
              ...
              if (completedTasksSet.has(dateStr)) {
                ...
                foundGap = true;
                console.log(
                  `[BONUS-PROGRESS] Bonus Day ${bonusDay.dayNumber}: Gap found at ${dateStr}, stopping count at ${consecutiveCompletedDays} consecutive days`,
                );
                break;
              }
            }
            ...
            if (consecutiveCompletedDays > 0) {
              progressPercentage = Math.min(
                100,
                Math.round(
                  (consecutiveCompletedDays / bonusDay.conditions.minStreak) *
                    100,
                ),
              );
              console.log(
                `[BONUS-PROGRESS] Bonus Day ${bonusDay.dayNumber}: ${consecutiveCompletedDays}/${bonusDay.conditions.minStreak} consecutive days ending today = ${progressPercentage}%${foundGap ? " (gap found, progress restarted from gap)" : ""}`,
              );
            } else {
              progressPercentage = 0; // No progress if no consecutive days ending today
              console.log(
                `[BONUS-PROGRESS] Bonus Day ${bonusDay.dayNumber}: Progress = 0% (no consecutive days ending today)`,
              );
            }
          }
        } else {
          ...
          progressPercentage = Math.min(
            100,
            Math.round((currentStreak / bonusDay.conditions.minStreak) * 100),
          );
          console.log(
            `[BONUS-PROGRESS] Bonus Day ${bonusDay.dayNumber}: Progress = ${progressPercentage}% (streak-based, requiresCompletion=false)`,
          );
        }
    ...
    const eligibleBonusDays = allBonusDays.filter((bd) =>
      bd.isEligibleForUser(userProfileForEligibility),
    );
...
function getResetRuleExplanation(resetRule) {
  ...
  if (!onMiss) {
    return {
      ...
      description:
        "Your streak will not be reset if you miss a day. You can continue from where you left off.",
  ...
  switch (fallbackAction) {
    case "pause_streak":
      description +=
        " Your streak will be paused (not reset) when you miss a day.";
      impact +=
        " Your streak count will remain but won't increase until you complete a day again.";
      break;
    case "give_alternate":
      description += " You'll receive an alternate reward if you miss a day.";
      impact +=
        " Missing a day will reset your streak but you'll still get a reward.";
      break;
    case "no_action":
    ...
    default:
      ...
      impact +=
        " You'll need to start over from day 1 to reach this bonus again.";
      break;
  }
...
async function checkGameTaskCompletionToday(userId, today) {
  try {
    console.log(
      "  [CHECK TASK] Checking if user completed any game task today...",
    );
    ...
    console.log(
      "  [CHECK TASK] Downloaded games count:",
    ...
    for (const game of downloadedGames) {
      ...
      if (game.goals && Array.isArray(game.goals)) {
        console.log(
          `  [CHECK TASK] Game ${gameIndex} (${game.title || game.id}): ${game.goals.length} goals`,
        );
        let goalIndex = 0;
        for (const goal of game.goals) {
          ...
          if (goal.completed === true && goal.completed_datetime) {
            ...
            try {
              ...
              console.log(
                `    [CHECK TASK] Goal ${goalIndex} (${goal.text || goal.goal_id}): completed_datetime = ${goal.completed_datetime}, date part = ${completedDateStr}`,
              );
              ...
              if (completedDateStr === todayDateStr) {
                console.log(
                  `    [CHECK TASK] ✅ FOUND! Goal ${goalIndex} was completed TODAY (${completedDateStr} === ${todayDateStr})`,
                );
                return true;
              } else {
                console.log(
                  `    [CHECK TASK] Goal ${goalIndex} was completed on ${completedDateStr}, not today (${todayDateStr})`,
                );
              }
            } catch (error) {
              // If date parsing fails, skip this goal
              console.error(
                `    [CHECK TASK] Error parsing completed_datetime for goal ${goalIndex}:`,
            ...
            if (goal.completed === true && !goal.completed_datetime) {
              console.log(
                `    [CHECK TASK] Goal ${goalIndex} is completed but has no completed_datetime`,
              );
            }
          }
        }
        ...
        console.log(
          `  [CHECK TASK] Game ${gameIndex} (${game.title || game.id}): No goals array`,
        );
      }
    }
...
router.get("/status", protect, async (req, res) => {
  try {
    ...
    const completedGameTaskToday = await checkGameTaskCompletionToday(
      req.user.userId,
      today,
    );
    ...
    if (isNewDay) {
      ...
      // Calculate actual missed days since last update
      const daysSinceLastUpdate = lastUpdate
        ? Math.floor((today - lastUpdate) / (1000 * 60 * 60 * 24))
        : 0;
      console.log(`Days since last update: ${daysSinceLastUpdate}`);

      if (completedGameTaskToday) {
        // Completed today: missed days = days before today (exclude today)
        const missedDays = Math.max(0, daysSinceLastUpdate - 1);
        const baseStreak = Math.max(0, currentStreakValue - missedDays);
        console.log(`Completed today — missed days: ${missedDays}, base: ${baseStreak}`);
        newStreakValue = baseStreak + 1;
        ...
        // Not completed today: today also counts as a missed day
        const missedDays = daysSinceLastUpdate;
        newStreakValue = Math.max(0, currentStreakValue - missedDays);
        console.log(`Not completed today — missed days (incl. today): ${missedDays}, new streak: ${newStreakValue}`);

        streak.completedTasks = streak.completedTasks.filter(
          (date) => date !== todayStr,
        );
        streak.lastUpdated = today;
        if (newStreakValue < currentStreakValue) {
          streak.resetAt = today;
          streak.resetReason = "missed_days";
        }
      }
      ...
      // Auto-award milestone reward if streak increased and hit a milestone (claimMode: auto)
      if (completedGameTaskToday && newStreakValue > currentStreakValue) {
        try {
          // Re-fetch full user document needed for wallet/xp updates
          const fullUser = await User.findById(req.user.userId).select('xp wallet dailyActivity streak');

          // Step 1: Check BonusDay model first (admin bonus days panel)
          let milestoneReward = null;
          const bonusDayDoc = await BonusDay.findByDayNumber(newStreakValue);
          if (bonusDayDoc && bonusDayDoc.isActive) {
            const bonusDayRewards = [];
            if (bonusDayDoc.primaryReward && bonusDayDoc.primaryReward.value > 0) {
              bonusDayRewards.push({ type: bonusDayDoc.primaryReward.type, value: bonusDayDoc.primaryReward.value });
            }
            if (bonusDayDoc.alternateReward && bonusDayDoc.alternateReward.value > 0) {
              bonusDayRewards.push({ type: bonusDayDoc.alternateReward.type, value: bonusDayDoc.alternateReward.value });
            }
            if (bonusDayRewards.length > 0) {
              milestoneReward = { rewards: bonusDayRewards, claimMode: 'auto' };
            }
          }

          // Step 2: Fall back to StreakBonusConfig (milestones 7/14/21/30)
          if (!milestoneReward) {
            milestoneReward = await getMilestoneRewardForUser(newStreakValue, STREAK_CONFIG, fullUser.country || null);
          }

          if (milestoneReward && milestoneReward.rewards && milestoneReward.rewards.length > 0 && ['auto', 'watch_ad'].includes(milestoneReward.claimMode)) {
            // Check awardedMilestones to prevent duplicates
            if (!fullUser.dailyActivity) fullUser.dailyActivity = { awardedMilestones: [] };
            if (!fullUser.dailyActivity.awardedMilestones) fullUser.dailyActivity.awardedMilestones = [];

            if (!fullUser.dailyActivity.awardedMilestones.includes(newStreakValue)) {
              let coinsReward = 0;
              let xpReward = 0;
              let finalXP = 0;
              ...
              for (const reward of milestoneReward.rewards) {
                if (reward.type === 'coins') {
                  ...
                  fullUser.wallet.balance = (fullUser.wallet.balance || 0) + reward.value;
                } else if (reward.type === 'xp') {
                  const xpResult = await applyTierMultiplierToXP(fullUser, reward.value);
                  finalXP = xpResult.finalXP;
                  tierMultiplier = xpResult.multiplier || 1.0;
                  xpReward = reward.value;
              ...
              if (hasCoins || hasXP) {
                ...
                // Duplicate prevention: check by user + milestoneDay + amount + today's date
                const startOfDay = new Date(todayStr + 'T00:00:00.000Z');
                const endOfDay   = new Date(todayStr + 'T23:59:59.999Z');
                const existingTransaction = await Transaction.findOne({
                  user: req.user.userId,
                  amount: primaryAmount,
                  'metadata.milestoneDay': newStreakValue,
                  'metadata.source': { $in: ['streak_bonus_auto', 'streak_bonus'] },
                  createdAt: { $gte: startOfDay, $lte: endOfDay }
                });

                if (existingTransaction) {
                  console.log(`⚠️ Duplicate transaction detected for Day ${newStreakValue} — amount: ${primaryAmount}, user: ${req.user.userId}, date: ${todayStr}. Skipping.`);
                } else {
                  const rewardParts = [];
                  ...
                  const transaction = new Transaction({
                    user: req.user.userId,
                    type: 'credit',
                    balanceType: hasCoins ? 'coins' : 'xp',
                    amount: primaryAmount,
                    description: `Streak Milestone Reward - Day ${newStreakValue} - ${rewardParts.join(' + ')}`,
                    status: 'completed',
                    referenceId: `STREAK-${newStreakValue}-${Date.now()}`,
                    metadata: {
                      milestoneDay: newStreakValue,
                      claimMode: milestoneReward.claimMode,
                      coins: hasCoins ? coinsReward : 0,
                      xp: hasXP ? finalXP : 0,
                      baseXp: hasXP ? xpReward : 0,
                      finalXp: hasXP ? finalXP : 0,
                      tierMultiplier: hasXP ? tierMultiplier : 1.0,
                      source: 'streak_bonus_auto'
                    }
                  });
                  await transaction.save();
                  console.log(`✅ Transaction created for Day ${newStreakValue} — amount: ${primaryAmount}, user: ${req.user.userId}, date: ${todayStr}`);
                }
              }

              // Mark milestone as awarded
              fullUser.dailyActivity.awardedMilestones.push(newStreakValue);
              await fullUser.save();
              console.log(`✅ Auto-awarded milestone reward for Day ${newStreakValue} via streak/status`);
            } else {
              console.log(`⚠️ Milestone Day ${newStreakValue} already awarded - skipping`);
            }
          }
        } catch (rewardErr) {
          console.error('Error auto-awarding milestone reward in streak/status:', rewardErr.message);
          // Don't fail the status response if reward awarding fails
        }
      }
    } else {
      ...
      if (completedGameTaskToday && !streak.completedTasks.includes(todayStr)) {
        ...
        !completedGameTaskToday &&
        streak.completedTasks.includes(todayStr)
      ) {
        console.log(
          "Task was marked as completed but actually not - removing today",
        );
    ...
    console.log(
      "user.streak.current (from DB after save):",
      (user.streak || {}).current,
    );
    ...
    const streakTree = generateStreakTree(
      currentStreak,
      STREAK_CONFIG,
      completedTasks,
    );
    ...
    streakTree.slice(0, 10).forEach((day) => {
      console.log(
        `  Day ${day.day}: isCompleted=${day.isCompleted}, isCurrent=${day.isCurrent}`,
      );
    });
    ...
    console.log(
      "Response streakTree days completed:",
      streakTree.filter((d) => d.isCompleted).length,
    );
...
router.post("/complete-task", protect, async (req, res) => {
  try {
    ...
    // Calculate missed days and adjust streak accordingly:
    // - Each missed day reduces streak by 1
    // - If missed days >= current streak → reset to 0
    // - If no missed days → streak continues normally
    const lastUpdated = streak.lastUpdated
      ? new Date(streak.lastUpdated)
      : null;
    let baseStreak = currentStreak;
    if (lastUpdated) {
      const daysSinceLastTask = Math.floor(
        (today - lastUpdated) / (1000 * 60 * 60 * 24),
      );
      const missedDays = daysSinceLastTask - 1; // subtract 1 because yesterday = no miss
      if (missedDays > 0) {
        baseStreak = Math.max(0, currentStreak - missedDays);
      }
    }
    ...
    const milestoneReward = await getMilestoneRewardForUser(
      newStreak,
      STREAK_CONFIG,
      userCountry,
    );
    ...
    if (
      ...
      const userWithActivity = await User.findById(req.user.userId).select(
        "dailyActivity",
      );
      ...
      if (!awardedMilestones.includes(newStreak)) {
        ...
        console.log(
          `✅ Task Completion Streak Milestone Reward Awarded: Day ${newStreak} - Prevented duplicate`,
        );
      } else {
        // Milestone already awarded - skip to prevent duplicate transactions
        console.log(
          `⚠️ Task Completion Streak Milestone Day ${newStreak} already awarded - skipping duplicate`,
        );
...
function getLastMilestone(currentStreak, STREAK_CONFIG) {
  const milestones = STREAK_CONFIG.milestones.filter(
    (day) => day <= currentStreak,
  );
...
async function getMilestoneRewardForUser(
  currentStreak,
  STREAK_CONFIG,
  userCountry,
) {
...
function generateStreakTree(currentStreak, STREAK_CONFIG, completedTasks = []) {
  ...
  console.log(
    "  [STREAK TREE] Today in completedTasks?",
  ...
  for (let day = 1; day <= STREAK_CONFIG.maxDays; day++) {
    ...
    if (day <= 10) {
      console.log(
        `  [STREAK TREE] Day ${day}: daysAgo=${daysAgo}, targetDate=${targetDateStr}, isCompleted=${isCompleted}`,
      );
    }
  ...
  console.log(
    "  [STREAK TREE] Days marked as completed:",
    tree.filter((d) => d.isCompleted).map((d) => d.day),
  );
...
function cleanupCompletedTasks(completedTasks, today) {
  ...
  console.log(
    "  [CLEANUP] Today in completedTasks?",
    completedSet.has(todayStr),
  );
  ...
  if (!completedSet.has(todayStr)) {
    console.log(
      "  [CLEANUP] Today NOT in completedTasks - streak broken, returning empty array",
    );
    return [];
  }
  ...
  while (true) {
    ...
    if (completedSet.has(dateStr)) {
      ...
      console.log(
        `  [CLEANUP] Day ${iteration + 1} ago (${dateStr}): FOUND - added to consecutive dates`,
      );
      checkDate.setDate(checkDate.getDate() - 1);
      iteration++;
    } else {
      console.log(
        `  [CLEANUP] Day ${iteration + 1} ago (${dateStr}): NOT FOUND - breaking streak`,
      );
      break;
    }
  }
...
function calculateConsecutiveStreak(completedTasks, today) {
  ...
  console.log(
    "  [CALCULATE] Today in completedTasks?",
    completedSet.has(todayStr),
  );
  ...
  while (true) {
    ...
    if (completedSet.has(dateStr)) {
      streak++;
      console.log(
        `  [CALCULATE] Day ${iteration + 1} ago (${dateStr}): FOUND - streak now = ${streak}`,
      );
      // Go back one day
      checkDate.setDate(checkDate.getDate() - 1);
      iteration++;
      ...
      console.log(
        `  [CALCULATE] Day ${iteration + 1} ago (${dateStr}): NOT FOUND - breaking streak`,
      );
      break;
    }
  }
