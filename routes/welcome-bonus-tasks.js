...
router.get("/", protect, async (req, res) => {
  try {
    ...
    console.log(`[Welcome Bonus Tasks] User has ${downloadedGames.length} downloaded games`);
    console.log(`[Welcome Bonus Tasks] Downloaded game IDs:`, downloadedGames.map(g => g.id));
    ...
    if (!bonusRule || !bonusRule.gameBonusTasks || bonusRule.gameBonusTasks.length === 0) {
      console.log(`[Welcome Bonus Tasks] No bonus rule or gameBonusTasks found`);
      return res.json({
        success: true,
        data: {
          games: [],
    ...
    console.log(`[Welcome Bonus Tasks] Bonus rule found with ${bonusRule.gameBonusTasks.length} game configurations`);
    console.log(`[Welcome Bonus Tasks] Configured game IDs:`, bonusRule.gameBonusTasks.map(c => String(c.gameId._id || c.gameId)));
    ...
    console.log(`[Welcome Bonus Tasks] User has ${userGames.length} games in user.games`);
    console.log(`[Welcome Bonus Tasks] User game IDs:`, userGames.map(g => String(g.gameId)));
    ...
    if (eligibleGameIdsForBonus.length === 0) {
      console.log(`[Welcome Bonus Tasks] ⚠️ User.games is empty. Matching downloaded games to DB for eligibility...`);
      const matchedGameIds = [];
      
      for (const downloadedGame of downloadedGames.slice(0, maxGamesWithBonus)) {
        ...
        if (gameDoc) {
          ...
          console.log(`[Welcome Bonus Tasks] Matched: ${downloadedGame.id} -> ${gameDoc._id.toString()}`);
        }
      }
      
      if (matchedGameIds.length > 0) {
        console.log(`[Welcome Bonus Tasks] Found ${matchedGameIds.length} eligible games by matching downloaded games to DB`);
        eligibleGameIdsForBonus = matchedGameIds;
      }
    }

    console.log(`[Welcome Bonus Tasks] Eligible games for bonus (first ${maxGamesWithBonus}):`, eligibleGameIdsForBonus);
    console.log(`[Welcome Bonus Tasks] Sorted games (first ${maxGamesWithBonus}):`, sortedGames.slice(0, maxGamesWithBonus).map(g => ({
    ...
    for (const downloadedGame of downloadedGames) {
      console.log(`[Welcome Bonus Tasks] Processing downloaded game: ${downloadedGame.title} (ID: ${downloadedGame.id})`);
      ...
      const gameBonusConfig = bonusRule.gameBonusTasks.find((config) => {
        ...
        if (gameGameId && gameGameId === downloadedGame.id) {
          console.log(`[Welcome Bonus Tasks] ✅ Matched by gameGameId: ${gameGameId} === ${downloadedGame.id}`);
          return true;
        }
      ...
      if (!gameBonusConfig) {
        console.log(`[Welcome Bonus Tasks] ⚠️ No bonus config found for game: ${downloadedGame.title} (Besitos ID: ${downloadedGame.id})`);
        console.log(`[Welcome Bonus Tasks] Available configs (gameGameId):`, bonusRule.gameBonusTasks.map(c => ({
      ...
      console.log(`[Welcome Bonus Tasks] ✅ Bonus config found for game: ${downloadedGame.title} with ${gameBonusConfig.bonusTasks.length} tasks`);
      ...
      if (gameDoc) {
        console.log(`[Welcome Bonus Tasks] ✅ Game found in DB: ${gameDoc.title} (DB ID: ${gameIdString}, Besitos ID: ${downloadedGame.id})`);
      } else {
        console.log(`[Welcome Bonus Tasks] ⚠️ Game not in DB but has bonus config: ${downloadedGame.title} (Besitos ID: ${downloadedGame.id})`);
      }
      ...
      if (userGames.length === 0) {
        ...
        console.log(`[Welcome Bonus Tasks] UserGames empty - showing games with bonus configs (current: ${gamesWithBonusTasks.length}/${maxGamesWithBonus})`);
        ...
        console.log(`[Welcome Bonus Tasks] Is eligible for bonus: ${isEligibleForBonus} (Game DB ID: ${gameIdString}, Eligible IDs: [${eligibleGameIdsForBonus.join(', ')}])`);
      }

      if (!isEligibleForBonus) {
        console.log(`[Welcome Bonus Tasks] ⚠️ Game ${downloadedGame.title} not eligible (not in first ${maxGamesWithBonus} games)`);
        continue;
      }

      // Limit to maxGamesWithBonus
      if (gamesWithBonusTasks.length >= maxGamesWithBonus) {
        console.log(`[Welcome Bonus Tasks] Reached max games limit (${maxGamesWithBonus}), stopping`);
        break;
      }

      if (!gameBonusConfig) {
        console.log(`[Welcome Bonus Tasks] ⚠️ No bonus config found for game: ${downloadedGame.title}`);
        console.log(`[Welcome Bonus Tasks] Looking for: DB ID=${gameIdString}, Besitos ID=${downloadedGame.id}`);
        console.log(`[Welcome Bonus Tasks] Available configs:`, bonusRule.gameBonusTasks.map(c => ({
          configGameId: String(c.gameId._id || c.gameId),
      ...
      // Find the first completed goal from Besitos API to start the countdown timer
      // Timer starts from when the first goal is completed, not from game download
      let sharedDeadlineStartTime = null;
      
      if (downloadedGame.goals && Array.isArray(downloadedGame.goals)) {
        // Find all completed goals and sort by position (order)
        const completedGoals = downloadedGame.goals
          .filter(goal => goal.completed === true && goal.completed_datetime)
          .sort((a, b) => {
            // Sort by position if available, otherwise by order in array
            const posA = a.position || a.order || 999;
            const posB = b.position || b.order || 999;
            return posA - posB;
          });
        
        if (completedGoals.length > 0) {
          // Use the first completed goal's completion time as the start
          const firstCompletedGoal = completedGoals[0];
          try {
            sharedDeadlineStartTime = new Date(firstCompletedGoal.completed_datetime);
            console.log(`[Welcome Bonus Tasks] ✅ Timer starts from first completed goal: "${firstCompletedGoal.text || firstCompletedGoal.goal_id}" at ${firstCompletedGoal.completed_datetime}`);
          } catch (error) {
            console.error(`[Welcome Bonus Tasks] ⚠️ Error parsing completed_datetime: ${firstCompletedGoal.completed_datetime}`, error);
            // Fallback to game download time if parsing fails
            sharedDeadlineStartTime = gameDownloadTime;
          }
        } else {
          // No goals completed yet - timer hasn't started
          console.log(`[Welcome Bonus Tasks] ⚠️ No goals completed yet for ${downloadedGame.title} - timer will start when first goal is completed`);
          sharedDeadlineStartTime = null; // Timer hasn't started yet
        }
      } else {
        // No goals data available - fallback to game download time
        console.log(`[Welcome Bonus Tasks] ⚠️ No goals data for ${downloadedGame.title} - using game download time`);
        sharedDeadlineStartTime = gameDownloadTime;
      }
      ...
      const formattedBonusTasks = await Promise.all(
        bonusTasksToProcess.map(async (bt, index) => {
          ...
          const taskBesitosGoalId = bt.taskId?.besitosGoalId || bt.taskId?.goalId || null;
          ...
          if (downloadedGame.goals && Array.isArray(downloadedGame.goals)) {
            ...
            if (taskBesitosGoalId) {
              matchingGoal = downloadedGame.goals.find(
                (goal) => (goal.goal_id === taskBesitosGoalId || goal.id === taskBesitosGoalId) && goal.completed === true
              );
            }
            ...
            if (!matchingGoal && taskName) {
              ...
              matchingGoal = downloadedGame.goals.find(
                (goal) => {
                  const goalText = (goal.text || goal.name || '').toLowerCase().trim();
                  // Exact match or contains key phrases
                  const isMatch = goalText === normalizedTaskName || 
                                 (normalizedTaskName.includes('installed') && goalText.includes('installed')) ||
                                 (normalizedTaskName.includes('install') && goalText.includes('install')) ||
                                 (normalizedTaskName.includes('reach level') && goalText.includes('reach level') && 
                                  normalizedTaskName.replace(/\D/g, '') === goalText.replace(/\D/g, '')); // Match level numbers
                  return isMatch && goal.completed === true;
                }
              );
              if (matchingGoal) {
                console.log(`[Welcome Bonus Tasks] ✅ Matched by name: "${taskName}" === "${matchingGoal.text || matchingGoal.name}"`);
              }
            }
          }
          ...
          if (matchingGoal) {
            ...
            console.log(`[Welcome Bonus Tasks] ✅ Task ${bt.order} (${taskName || 'Unknown'}) completed via Besitos API (goal_id: ${matchingGoal.goal_id || matchingGoal.id}, text: ${matchingGoal.text || 'N/A'})`);
            ...
            if (!userTask || !userTask.completed) {
              ...
              const existingUserTaskIndex = user.tasks.findIndex(
                (t) => t.taskId && (t.taskId.toString() === taskIdString || String(t.taskId) === taskIdString)
              );
              ...
              if (existingUserTaskIndex >= 0) {
                ...
                user.tasks[existingUserTaskIndex].completedAt = completedDateTime;
                if (!user.tasks[existingUserTaskIndex].date) {
                  user.tasks[existingUserTaskIndex].date = completedDateTime;
                }
                console.log(`[Welcome Bonus Tasks] 📝 Updated user task status for task ${bt.order} (${taskName || 'Unknown'})`);
              } else {
                // Create new task entry
                user.tasks.push({
                  taskId: mongoose.Types.ObjectId.isValid(taskId) ? taskId : mongoose.Types.ObjectId(taskIdString),
                  completed: true,
                  completedAt: completedDateTime,
                  date: completedDateTime,
                });
                console.log(`[Welcome Bonus Tasks] 📝 Created new user task entry for task ${bt.order} (${taskName || 'Unknown'})`);
              }
            }
          }
          ...
          if (index === 0) {
            ...
            const previousTaskBesitosGoalId = previousTask?.taskId?.besitosGoalId || previousTask?.taskId?.goalId || null;
            ...
            if (downloadedGame.goals && Array.isArray(downloadedGame.goals)) {
              ...
              if (previousTaskBesitosGoalId) {
                previousMatchingGoal = downloadedGame.goals.find(
                  (goal) => (goal.goal_id === previousTaskBesitosGoalId || goal.id === previousTaskBesitosGoalId) && goal.completed === true
                );
              }
              
              // SECONDARY: Match by name/text
              if (!previousMatchingGoal && previousTaskName) {
                const normalizedTaskName = previousTaskName.toLowerCase().trim();
                previousMatchingGoal = downloadedGame.goals.find(
                  (goal) => {
                    const goalText = (goal.text || goal.name || '').toLowerCase().trim();
                    const isMatch = goalText === normalizedTaskName || 
                                   ...
                                   (normalizedTaskName.includes('install') && goalText.includes('install')) ||
                                   (normalizedTaskName.includes('reach level') && goalText.includes('reach level') && 
                                    normalizedTaskName.replace(/\D/g, '') === goalText.replace(/\D/g, ''));
                    return isMatch && goal.completed === true;
                  }
                );
              }
          ...
          // Calculate shared completion deadline: All tasks share the same deadline starting from first goal completion time
          // Timer starts from first completed goal's completed_datetime from Besitos API
          // If timer hasn't started (no goals completed yet), deadline is null
      ...
      // Calculate completion deadline for reward checking
      // Timer starts from first completed goal's completed_datetime from Besitos API
      // Deadline = first goal completion time + completionDeadlineHours (configured by admin)
      const completionDeadline = sharedDeadlineStartTime
        ? new Date(
            sharedDeadlineStartTime.getTime() +
              completionDeadlineHours * 60 * 60 * 1000
      ...
      // Check if ALL tasks are completed and award reward per game (not per task)
      // Reward: 100 coins + 20 XP total per game (regardless of number of tasks)
      // All tasks must be completed within the deadline (from first goal completion time)
      const allTasksCompleted = formattedBonusTasks.every(task => task.isCompleted);
      const allTasksCompletedWithinDeadline = allTasksCompleted && completionDeadline && sharedDeadlineStartTime
        ? formattedBonusTasks.every(task => {
            if (!task.completedAt) return false;
            const completedDate = new Date(task.completedAt);
            return completedDate <= completionDeadline;
          })
        : false;
      ...
      if (allTasksCompletedWithinDeadline) {
        ...
        if (!existingGameReward) {
          ...
          const coinsTransaction = new Transaction({
            ...
            metadata: {
              ...
              completedTasks: formattedBonusTasks.filter(t => t.isCompleted).length,
          ...
          console.log(`[Welcome Bonus Tasks] ✅ Awarded game reward for ${downloadedGame.title}: ${coinsReward} coins + ${finalXP} XP (all ${formattedBonusTasks.length} tasks completed)`);
          ...
          console.log(`[Welcome Bonus Tasks] ⚠️ Game reward already awarded for ${downloadedGame.title}`);
        }
        ...
        console.log(`[Welcome Bonus Tasks] ❌ All tasks completed for ${downloadedGame.title} but after deadline`);
      }
      ...
      gamesWithBonusTasks.push({
        ...
        gameStatus: inProgressGameIds.has(downloadedGame.id) ? "in_progress" : "completed",
        minimumEventThreshold: minimumEventThreshold,
        completionDeadlineHours: completionDeadlineHours,
        ...
        userDownloadOrder:
          sortedGames.findIndex(
            (g) => String(g.gameId) === gameIdString
          ) + 1,
      });
    }
    ...
    if (hasRewards || hasTaskUpdates) {
      await user.save();
      if (hasRewards) {
        console.log(`[Welcome Bonus Tasks] Saved user with ${rewardsAwarded.length} rewards awarded`);
      }
      if (hasTaskUpdates) {
        const completedTasksCount = user.tasks?.filter(t => t.completed).length || 0;
        console.log(`[Welcome Bonus Tasks] Saved user with updated task statuses (${completedTasksCount} completed tasks from Besitos goals)`);
      }
    }
    ...
    const debugInfo = gamesWithBonusTasks.length === 0 ? {
      ...
      bonusConfigGameIds: bonusRule?.gameBonusTasks?.map(c => String(c.gameId._id || c.gameId)) || [],
    } : null;
    ...
    if (debugInfo) {
      ...
      console.log(`[Welcome Bonus Tasks] ⚠️ No games returned. Debug info:`, JSON.stringify(debugInfo, null, 2));
    }
