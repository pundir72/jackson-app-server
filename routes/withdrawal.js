    ...
    res.json({
      ...
      data: {
        ...
        shortfall: isEligible
          ? 0
          : (WITHDRAWAL_CONFIG.minAmount - usdBalance).toFixed(2),
      },
    });
  } catch (error) {
...
router.post("/request", protect, async (req, res) => {
  try {
    ...
    if (
      amount < WITHDRAWAL_CONFIG.minAmount ||
      amount > WITHDRAWAL_CONFIG.maxAmount
    ) {
      return res.status(400).json({
        success: false,
    ...
    // Create withdrawal request
    const withdrawalId = `WD-${Date.now()}-${Math.floor(
      Math.random() * 100000
    )}`;
  ...
  try {
    ...
    if (action === "approve") {
      ...
      user.redemption.totalCoinsRedeemed =
        (user.redemption.totalCoinsRedeemed || 0) + withdrawal.requiredCoins;
      user.redemption.lastRedeemedAt = new Date();
      user.redemption.preference =
        withdrawal.method || user.redemption.preference || "none";
