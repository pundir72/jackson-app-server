...
class BesitosService {
    ...
    async getOffers(queryParams = {}) {
        ...
        try {
            ...
            console.error('Besitos getOffers error:', error);
            throw error;
        }
    }
    ...
    async healthCheck() {
        ...
        try {
            const response = await this.client.get(`/data/partner/offers/${this.partnerId}`, {
                params: { limit: 1 }
            });
            ...
            return {
                status: 'ok',
                configured: true,
                data: { reachable: true, sampleCount: Array.isArray(response.data) ? response.data.length : 1 }
            };
        } catch (error) {
