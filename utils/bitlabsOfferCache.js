...
class BitlabsOfferCache {
    ...
    async getOffers(queryParams = {}) {
        ...
        console.log(`Bitlabs offers cache miss, fetching fresh data (key: ${cacheKey})`);
    ...
    async refreshOffers(queryParams = {}) {
        ...
        try {
            ...
            if (result && result.success && Array.isArray(result.data)) {
                ...
                console.log(`Bitlabs offers refreshed and cached: ${result.data.length} offers (key: ${cacheKey})`);
                return result.data;
            } else if (Array.isArray(result)) {
                // If service returns array directly
                cache.set(cacheKey, result);
                this.lastRefreshTime = new Date();
                console.log(`Bitlabs offers refreshed and cached: ${result.length} offers (key: ${cacheKey})`);
                return result;
            } else {
                console.error('Bitlabs offers refresh failed: Invalid response format', result);
                return [];
            }
        } catch (error) {
    ...
    startPeriodicRefresh(intervalMinutes = null) {
        const interval = intervalMinutes || config.BITLABS_REFRESH_INTERVAL_MINUTES || 5;
        ...
        this.refreshInterval = setInterval(() => {
            console.log(`Periodic Bitlabs offer refresh triggered (every ${interval} minutes)`);
            this.refreshOffers().catch(err => {
                console.error('Error in periodic Bitlabs offer refresh:', err);
            });
        }, intervalMs);

        console.log(`Bitlabs offer cache periodic refresh started (every ${interval} minutes)`);
    }
    ...
    async preFetchCommonOffers() {
        ...
        for (const query of commonQueries) {
            try {
                ...
                console.error(`Error pre-fetching offers for ${JSON.stringify(query)}:`, error.message);
            }
        }
