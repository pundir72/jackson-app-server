...
async function loadProgressFixed(userId, dateUtc = new Date()) {
  try {
    ...
    console.log(
      `   📅 Using CALENDAR WEEK logic (all weeks use calendar-based days)`,
    );
    ...
    if (weekContainsUserCreation) {
      ...
      console.log(
        `   📍 User joined on calendar Day ${userCreatedDayNumber} (${userCreatedDayIdx === 0 ? "Monday" : userCreatedDayIdx === 1 ? "Tuesday" : userCreatedDayIdx === 2 ? "Wednesday" : userCreatedDayIdx === 3 ? "Thursday" : userCreatedDayIdx === 4 ? "Friday" : userCreatedDayIdx === 5 ? "Saturday" : "Sunday"})`,
      );
      console.log(
        `   ⚠️  Days 1-${userCreatedDayIdx} (before join) will be marked as MISSED/UNCLAIMED`,
      );
    }
      ...
      if (isCurrentWeek) {
        ...
        progress.days.forEach((d, idx) => {
          ...
          if (weekContainsUserCreation && idx < userCreatedDayIdx) {
            ...
            changed = true;
            console.log(
              `   ❌ Day ${idx + 1} marked as MISSED (before user join)`,
            );
          }
          // Days after user creation but before today are missed
          else if (
            weekContainsUserCreation &&
            idx >= userCreatedDayIdx &&
            idx < todayIdx
          ) {
            d.status = "missed";
            changed = true;
          ...
          // Today is claimable (only if it's on or after user creation day)
            ...
            if (weekContainsUserCreation && idx >= userCreatedDayIdx) {
              ...
              changed = true;
              console.log(
                `   ✅ Day ${idx + 1} marked as CLAIMABLE (user's join day or later)`,
              );
    ...
    progress.days.forEach((d, idx) => {
      ...
      if (weekContainsUserCreation && idx < userCreatedDayIdx) {
        if (d.status !== "missed") {
          ...
          changed = true;
          console.log(
            `   🔒 Day ${idx + 1} forced to MISSED (before user join)`,
          );
        }
 ...
 */
function calculateMidWeekJoinMetadataFixed(
  weekStart,
  weekEnd,
  userCreatedAt,
  isFirstWeek = false,
  ...
  if (!weekContainsUserCreation) {
    return {
      ...
      message:
        "This week follows the standard calendar-based daily reward cycle.",
      explanation:
        "Daily rewards follow the calendar week (Monday-Sunday). Days before your join date are marked as missed/unclaimed.",
